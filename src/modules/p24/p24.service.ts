...
export const handleP24Notification = async (payload: {
  ...
  const booking = await p24Repository.findBookingByP24SessionId(
    payload.sessionId,
  );
  if (!booking) throw new AppError("Booking not found for P24 session", 404);
  ...
  if (!ok) throw new AppError("P24 transaction verification failed", 400);
