...
const verifyCaptcha = async (token) => {
    if (!token)
        return true; // Skip if frontend doesn't send a token
    const secretKey = process.env.RECAPTCHA_SECRET_KEY;
    if (!secretKey)
        return true; // Skip if you haven't configured the secret key in .env yet
    try {
...
export const getAllContactsService = async (page = 1, limit = 10, search, status) => {
